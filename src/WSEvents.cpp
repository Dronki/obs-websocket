...
bool transitionIsCut(obs_source_t *transition)
{
	...
	if (obs_source_get_type(transition) == OBS_SOURCE_TYPE_TRANSITION && QString(obs_source_get_id(transition)) == "cut_transition")
	{
		return true;
	}
	return false;
...
{
	WSEvents *owner = (WSEvents *)private_data;
...
{
	WSEvents *instance = (WSEvents*)param;
